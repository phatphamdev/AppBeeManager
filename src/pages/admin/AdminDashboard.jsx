...
import {
  ...
  Tabs, Tab, Alert, Chip, CircularProgress,
} from '@mui/material';
...
const gridSx = {
  ...
  '& .dx-datagrid-headers .dx-datagrid-table .dx-row > td': {
    backgroundColor: 'rgba(241,240,239,0.04)', color: '#a09d9a',
    fontWeight: 600, fontSize: '0.75rem', textTransform: 'uppercase',
    letterSpacing: '0.05em', borderBottom: '1px solid rgba(241,240,239,0.1)',
  },
...
function ServicesGrid() {
  ...
  const onRowUpdated = async (e) => {
    ...
    if (err) setError(`Lỗi cập nhật: ${err.message}`);
    else fetchData();
  };
  const onRowRemoved = async (e) => {
    const { error: err } = await supabase.from('services').delete().eq('id', e.data.id);
    ...
    <Box>
      ...
      <Box sx={gridSx}>
        <DataGrid ref={gridRef} dataSource={data} keyExpr="id" showBorders={false} showColumnLines={false} showRowLines columnAutoWidth
          ...
          <Toolbar><ToolbarItem name="addRowButton" showText="always" /><ToolbarItem name="revertButton" /><ToolbarItem name="saveButton" /></Toolbar>
...
function SurchargesGrid() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    const { data: rows, error: err } = await supabase.from('surcharges').select('*').order('id');
    if (err) setError(err.message); else setData(rows || []);
    setLoading(false);
  }, []);
  ...
  const onRowUpdated = async (e) => {
    const { id, ...payload } = e.data;
    const { error: err } = await supabase.from('surcharges').update(payload).eq('id', id);
    if (err) setError(`Lỗi cập nhật: ${err.message}`);
    else fetchData();
  };
  const onRowRemoved = async (e) => {
    const { error: err } = await supabase.from('surcharges').delete().eq('id', e.data.id);
    if (err) setError(`Lỗi xóa: ${err.message}`);
    else fetchData();
  };
  ...
  return (
    <Box>
      {error && <Alert severity="error" onClose={() => setError('')} sx={{ mb: 2 }}>{error}</Alert>}
      <Box sx={gridSx}>
        <DataGrid dataSource={data} keyExpr="id" showBorders={false} showColumnLines={false} showRowLines columnAutoWidth
          onRowInserted={onRowInserted} onRowUpdated={onRowUpdated} onRowRemoved={onRowRemoved}>
          <Editing mode="row" allowAdding allowUpdating allowDeleting confirmDelete useIcons />
          <FilterRow visible /><Sorting mode="multiple" /><Paging defaultPageSize={10} />
          <Pager showPageSizeSelector allowedPageSizes={[5,10,20]} showInfo infoText="Trang {0}/{1} ({2} bản ghi)" />
          ...
          <Column dataField="type" caption="Loại" width={140} cellRender={({value})=><TypeCell value={value}/>} validationRules={[{type:'required'}]}>
            <Lookup dataSource={['FIXED','MULTIPLIER']} />
          </Column>
          <Column dataField="value" caption="Giá trị" dataType="number" alignment="right" format={{type:'fixedPoint',precision:2}} validationRules={[{type:'required'},{type:'numeric'}]} />
          <Column dataField="is_active" caption="Trạng thái" dataType="boolean" width={130} alignment="center" cellRender={({value})=><ActiveCell value={value}/>} />
        </DataGrid>
      </Box>
    </Box>
  );
...
export default function AdminDashboard() {
  ...
  return (
    <Box className="page-enter" sx={{ minHeight: 'calc(100vh - 64px)', background: 'radial-gradient(ellipse at 80% 0%, rgba(6,182,212,0.05) 0%, transparent 60%)', py: 3 }}>
      <Container maxWidth="xl">
        <Box sx={{ mb: 3 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 0.5 }}>
            ...
            <Typography variant="h5" fontWeight={800} letterSpacing="-0.5px">Quản Trị Hệ Thống</Typography>
          </Box>
        ...
        <Card variant="outlined">
          <Box sx={{ borderBottom: '1px solid rgba(241,240,239,0.08)' }}>
            <Tabs value={activeTab} onChange={(_, v) => setActiveTab(v)} sx={{
              px: 2,
              '& .MuiTab-root': { fontWeight: 600, fontSize: '0.875rem', color: 'text.secondary', textTransform: 'none', minHeight: 52, gap: 1 },
              '& .Mui-selected': { color: 'primary.main' },
              '& .MuiTabs-indicator': { backgroundColor: 'primary.main', height: 3, borderRadius: 2 },
            }}>
              <Tab icon={<LocalTaxiRoundedIcon fontSize="small" />} iconPosition="start" label="Dịch vụ" />
              <Tab icon={<TuneRoundedIcon fontSize="small" />}      iconPosition="start" label="Phụ phí" />
            </Tabs>
          </Box>
          <CardContent sx={{ p: { xs: 2, md: 3 }, '&:last-child': { pb: 3 } }}>
            <Alert severity="info" variant="outlined" sx={{ mb: 3, fontSize: '0.82rem', borderColor: 'rgba(6,182,212,0.3)', color: 'secondary.light' }}>
              💡 Nhấn <strong>+ Thêm mới</strong> để tạo bản ghi. Nhấn biểu tượng bút chì để chỉnh sửa, thùng rác để xóa.
            </Alert>
            <div role="tabpanel" hidden={activeTab !== 0}>{activeTab === 0 && <ServicesGrid />}</div>
            <div role="tabpanel" hidden={activeTab !== 1}>{activeTab === 1 && <SurchargesGrid />}</div>
          </CardContent>
        </Card>
      </Container>
    </Box>
